...
func (c *GfCache) cacheTagKey(ctx context.Context, key interface{}, tag string) {
	tagKey := c.CachePrefix + c.getTagKey(tag)
	if tagKey != "" {
		tagVal := []interface{}{key}
		...
		if value != nil {
			...
			for _, v := range keyValue {
				if !reflect.DeepEqual(key, v) {
					tagVal = append(tagVal, v)
				}
			}
		}
		c.cache.Set(ctx, tagKey, tagVal, 0)
	}
}

// 获取带标签的键名
func (c *GfCache) getTagKey(tag string) string {
	if tag != "" {
		tag = "tag_" + tag
	}
	return tag
